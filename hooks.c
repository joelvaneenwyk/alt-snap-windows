...
static struct config {
    ...
    struct {
        enum action  // Up to 20 BUTTONS!!!
            LMB[NACPB],
            RMB[NACPB], MMB[NACPB], MB4[NACPB], MB5[NACPB], MB6[NACPB], MB7[NACPB], MB8[NACPB], MB9[NACPB], MB10[NACPB],
            MB11[NACPB], MB12[NACPB], MB13[NACPB], MB14[NACPB], MB15[NACPB], MB16[NACPB], MB17[NACPB], MB18[NACPB],
            MB19[NACPB], MB20[NACPB], Scroll[NACPB], HScroll[NACPB];  // Plus vertical and horizontal wheels
    } Mouse;
...
static const char *BlackListStrings[] = {"Processes",  // Max length is 15 char + NULL
                                         "Windows",   "Snaplist", "MDIs",      "Pause",     "MMBLower",  "Scroll",
                                         "IScroll",   "AResize",  "SSizeMove", "NCHittest", "Bottommost"};
// Cursor data
...
static pure int blacklisted(HWND hwnd, const struct blacklist *list)
{
    ...
    UCHAR    mode = list->items[0].classname || list->items[0].title || list->items[0].exename;
    ...
    for (; i < list->length; i++) {
        if (!lstrcmp_star(classname, list->items[i].classname) && !lstrcmp_star(title, list->items[i].title)
...
{
    TCHAR classname[256];
    return GetClassName(hwnd, classname, ARR_SZ(classname)) && !lstrcmp(classname, str);
}
/////////////////////////////////////////////////////////////////////////////
...
static int pure IsResizable(HWND hwnd)
{
    ...
    if (!ret) {
        ret = !!GetBorderlessFlag(hwnd);
    }
    if (!ret) {
        ret = !!blacklisted(hwnd, &BlkLst.AResize);  // Always resize list
    }
...
static int IsFullscreenF(HWND hwnd, const RECT *wnd, const RECT *fmon)
{
    ...
    return ((style & WS_CAPTION) != WS_CAPTION) && EqualRect(wnd, fmon) == TRUE;
}
static int  IsFullScreenBL(HWND hwnd) { return conf.FullScreen && IsFullscreen(hwnd); }
/////////////////////////////////////////////////////////////////////////////
...
static int HitTestTimeoutbl(HWND hwnd, POINT pt)
{
    ...
    if (blacklisted(ancestor, &BlkLst.MMBLower)) {
        return 0;
    }
    if (hwnd != ancestor && blacklisted(ancestor, &BlkLst.NCHittest)) {
        SendMessageTimeout(ancestor, WM_NCHITTEST, 0, MAKELPARAM(pt.x, pt.y), SMTO_NORMAL, 200, &area);
        if (area == HTCAPTION) {
            goto DOUBLECHECK_CAPTION;
        }
    }
...
/////////////////////////////////////////////////////////////////////////////
//
static void GetMinMaxInfo(HWND hwnd, POINT *Min, POINT *Max) { GetMinMaxInfoF(hwnd, Min, Max, conf.IgnoreMinMaxInfo); }
/////////////////////////////////////////////////////////////////////////////
...
static void SetWindowTrans(HWND hwnd)
{
    ...
    static HWND oldhwnd;
    if (conf.MoveTrans == 0 || conf.MoveTrans == 255) {
        return;
    }
    if (oldhwnd == hwnd) {
...
static void *GetEnoughSpace(void *ptr, unsigned num, unsigned *alloc, size_t size)
{
    if (num >= *alloc) {
        ...
        ptr = nptr;
        if (ptr) {
            *alloc = (*alloc + 4);  // Realloc succeeded, increase count.
        } else {
            *alloc = 0;
        }
    }
...
/////////////////////////////////////////////////////////////////////////////
static void OffsetRectMDI(RECT *wnd) { OffsetRect(wnd, -state.mdipt.x, -state.mdipt.y); }
static int  ShouldSnapTo(HWND hwnd)
{
    LONG_PTR style;
    return hwnd != state.hwnd && IsVisible(hwnd) && !IsIconic(hwnd)
           && !(GetWindowLongPtr(hwnd, GWL_EXSTYLE) & WS_EX_NOACTIVATE)  // != WS_EX_NOACTIVATE
           && (((style = GetWindowLongPtr(hwnd, GWL_STYLE)) & WS_CAPTION) == WS_CAPTION || (style & WS_THICKFRAME)
...
BOOL CALLBACK EnumTouchingWindows(HWND hwnd, LPARAM lParam)
{
    ...
    RECT wnd;
    if (ShouldSnapTo(hwnd) && !IsZoomed(hwnd) && IsResizable(hwnd) && !blacklisted(hwnd, &BlkLst.Windows)
...
static int ShouldResizeTouching()
{
    return state.action == AC_RESIZE
           && ((conf.StickyResize & 1 && state.shift) || ((conf.StickyResize & 3) == 2 && !state.shift));
}
...
static int  ResizeTouchingWindows(LPVOID lwptr)
{
    ...
    for (i = 0; i < numsnwnds; i++) {
        ...
        if (hwndSS) {
            RECT nbd;
            FixDWMRect(hwnd, &nbd);
            hwndSS = DeferWindowPos(hwndSS, hwnd, NULL, nwnd->left - nbd.left, nwnd->top - nbd.top,
    ...
    if (hwndSS) {
        ...
        hwndSS = DeferWindowPos(hwndSS, state.hwnd, NULL, lw->x, lw->y, lw->width, lw->height,
                                SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOOWNERZORDER);
        if (hwndSS) {
            EndDeferWindowPosAsync(hwndSS);
        }
    }
    return 1;
}
...
{
    if (!conf.StickyResize || !numsnwnds) {
        return;
    }
    ...
    for (i = 0; i < numsnwnds; i++) {
        if (hwndSS && snwnds[i].flag & TORESIZE) {
            ...
            hwndSS = DeferWindowPos(hwndSS, snwnds[i].hwnd, NULL, snwnds[i].wnd.left, snwnds[i].wnd.top,
                                    snwnds[i].wnd.right - snwnds[i].wnd.left, snwnds[i].wnd.bottom - snwnds[i].wnd.top,
                                    SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOOWNERZORDER);
        }
    }
    if (hwndSS) {
        hwndSS = DeferWindowPos(hwndSS, LastWin.hwnd, NULL, LastWin.x, LastWin.y, LastWin.width, LastWin.height,
                                SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOOWNERZORDER);
    }
    if (hwndSS) {
        EndDeferWindowPosAsync(hwndSS);
...
{
    RECT *bd;
    if (!state.snap || state.Speed > conf.AeroMaxSpeed) {
        return;
    }
    EnumOnce(&bd);
    ...
    for (i = 0, j = 0; i < nummonitors || j < numwnds;) {
        ...
        if (IsInRangeT(posy, snapwnd.top, snapwnd.bottom, thresholdx)
            ...
            UCHAR snapinside_cond =
                (snapinside || posy + wndheight - thresholdx < snapwnd.top || snapwnd.bottom < posy + thresholdx);
        ...
        if (IsInRangeT(posx, snapwnd.left, snapwnd.right, thresholdy)
            ...
            UCHAR snapinside_cond =
                (snapinside || posx + wndwidth - thresholdy < snapwnd.left || snapwnd.right < posx + thresholdy);
...
static void ResizeSnap(int *posx, int *posy, int *wndwidth, int *wndheight, UCHAR pthx, UCHAR pthy)
{
    ...
    for (i = 0, j = 0; i < nummonitors || j < numwnds;) {
        ...
        if (IsInRangeT(*posy, snapwnd.top, snapwnd.bottom, thresholdx)
            ...
            UCHAR snapinside_cond =
                snapinside || (*posy + *wndheight - thresholdx < snapwnd.top) || (snapwnd.bottom < *posy + thresholdx);
            if (state.resize.x == RZ_LEFT && IsEqualT(snapwnd.right, *posx, thresholdx)) {
                // The left edge of the dragged window will snap to this window's right edge
                stuckleft  = 1;
                ...
                thresholdx = snapwnd.right - (*posx + *wndwidth);
            } else if (snapinside_cond && state.resize.x == RZ_LEFT && IsEqualT(snapwnd.left, *posx, thresholdx)) {
                // The left edge of the dragged window will snap to this window's left edge
                stuckleft  = 1;
                stickleft  = snapwnd.left;
                thresholdx = snapwnd.left - *posx;
            } else if (state.resize.x == RZ_RIGHT && IsEqualT(snapwnd.left, *posx + *wndwidth, thresholdx)) {
                // The right edge of the dragged window will snap to this window's left edge
                stuckright = 1;
        ...
        if (IsInRangeT(*posx, snapwnd.left, snapwnd.right, thresholdy)
            ...
            UCHAR snapinside_cond =
                snapinside || (*posx + *wndwidth - thresholdy < snapwnd.left) || (snapwnd.right < *posx + thresholdy);
            if (state.resize.y == RZ_TOP && IsEqualT(snapwnd.bottom, *posy, thresholdy)) {
                // The top edge of the dragged window will snap to this window's bottom edge
                stucktop   = 1;
                ...
                thresholdy  = snapwnd.bottom - (*posy + *wndheight);
            } else if (snapinside_cond && state.resize.y == RZ_TOP && IsEqualT(snapwnd.top, *posy, thresholdy)) {
                // The top edge of the dragged window will snap to this window's top edge
                stucktop   = 1;
                sticktop   = snapwnd.top;
                thresholdy = snapwnd.top - *posy;
            } else if (state.resize.y == RZ_BOTTOM && IsEqualT(snapwnd.top, *posy + *wndheight, thresholdy)) {
                // The bottom edge of the dragged window will snap to this window's top edge
                stuckbottom = 1;
...
static void MaximizeRestore_atpt(HWND hwnd, UINT sw_cmd, int origin)
{
    ...
    if (sw_cmd == SW_FULLSCREEN) {
        MoveWindowAsync(hwnd, mi.rcMonitor.left, mi.rcMonitor.top, mi.rcMonitor.right - mi.rcMonitor.left,
...
static void MoveResizeWindowThread(struct windowRR *lw, UINT flag)
{
    ...
    if (lw->end && conf.FullWin) {
        Sleep(8);  // At the End of movement...
    }
    if (lw->end && !lw->maximize && (IsZoomed(hwnd) || IsWindowSnapped(hwnd))) {
        ...
        if (conf.RefreshRate) {
            ASleep(conf.RefreshRate);  // Accurate!!!
        }
    }
    lw->hwnd = NULL;
    lw->end  = 0;
...
static DWORD WINAPI MoveWindowThread(LPVOID LastWinV)
{
    ...
    //        }
    //        //LOGA("MV: %d:%d/%d -> %d:%d/%d %s", cW, cH, cdpi , lw->width, lw->height, lw->odpi,
    //        notsamesize?"(dif)":"(eq)");
    //    }
    ...
    if (conf.FullWin) {
        ...
        if (lw->odpi == GetDpiForWindow(lw->hwnd) && GetWindowRect(lw->hwnd, &rc)) {
            int cW      = rc.right - rc.left;
            int cH      = rc.bottom - rc.top;
            nothingtodo = rc.left == lw->x && rc.top == lw->y && cW == lw->width && cH == lw->height;
        }
...
{
    if (state.mdiclient && GetClientRect(state.mdiclient, _mon)) {
        return;  // MDI!
    }
...
static int AeroMoveSnap(POINT pt, int *posx, int *posy, int *wndwidth, int *wndheight)
{
    ...
    if ((!conf.Aero && !(conf.UseZones & 1)) || !state.resizable) {
        return 0;
    }
    ...
    if (pt.y < Top && pt.x < Left) {
        ...
        *posy      = mon.bottom - *wndheight;
        ...
        // Pure Top
        if (!state.shift ^ !(conf.AeroTopMaximizes & 1) && (state.Speed < conf.AeroMaxSpeed)) {
            if (conf.FullWin) {
...
static void HideCursor()
{
    ...
    SetWindowPos(g_mainhwnd, NULL, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOREDRAW | SWP_DEFERERASE);
...
static pure int    ModKey() { return conf.ModKey[0] && IsAKeyDown(conf.ModKey); }
///////////////////////////////////////////////////////////////////////////
...
{
    return (0x41 <= a && a <= 0x5A)  // A-Z vkeys
           || IsHotkeyy(a, conf.Killkey);
}
static xpure int IsModKey(const UCHAR vkey) { return IsHotkeyy(vkey, conf.ModKey); }
...
{
    mdicpt->x = mdicpt->y = 0;
    if (!GetClientRect(state.mdiclient, wnd) || !ClientToScreen(state.mdiclient, mdicpt)) {
        return FALSE;
    }
    return TRUE;
}
...
static void ShowTransWin(int nCmdShow)
{
    if (conf.TransWinOpacity) {
        if (g_transhwnd[0]) {
            ShowWindow(g_transhwnd[0], nCmdShow);
        }
    } else {
        int i;
        for (i = 0; i < 4; i++) {
            if (g_transhwnd[i]) {
                ShowWindow(g_transhwnd[i], nCmdShow);
            }
        }
    }
}
...
static void MouseMove(POINT pt)
{
    ...
    if (!state.moving && !GetWindowRect(state.hwnd, &wnd)) {
        return;
    }
    ...
    if (state.action == AC_MOVE) {
        ...
        if (!LastWin.snap && (was_snapped || IsZoomed(state.hwnd))) {
            ...
            if (restore & SNTHENROLLED) {
                ClearRestoreData(state.hwnd);  // Only Flag?
            }
...
static DWORD WINAPI Send_ClickProc(LPVOID buttonD)
{
    ...
    static const WORD bmapping[] = {MOUSEEVENTF_LEFTDOWN, MOUSEEVENTF_RIGHTDOWN, MOUSEEVENTF_MIDDLEDOWN,
                                    MOUSEEVENTF_XDOWN, MOUSEEVENTF_XDOWN};
    if (!button || button > BT_MB5) {
        return 0;
    }
...
static void HotkeyUp()
{
    ...
    state.alt  = 0;
    state.alt1 = 0;
    if (state.action && (conf.GrabWithAlt[0] || conf.GrabWithAlt[1])
...
static void ReallySetForegroundWindow(HWND hwnd)
{
    ...
    if (fore != hwnd) {
        if (state.alt != VK_MENU && state.alt != VK_CONTROL && !(GetKeyState(VK_CONTROL) & 0x8000)
...
static int ScrollLockState()
{
    ...
    if ((conf.ScrollLockState & 1) && !(!(GetKeyState(VK_SCROLL) & 1) ^ !(conf.ScrollLockState & 2))) {
        if (state.action) {
            FinishMovement();
        }
        return 1;
...
static void LogState(const char *Title)
{
    ...
    fprintf(f,
            ...
            "blockmouseup=%d\n"
            "ignorekey=%d\n\n\n",
            (int)state.clickbutton, (DWORD)(DorQWORD)state.hwnd, (DWORD)(DorQWORD)LastWin.hwnd, (int)LastWin.end,
            (int)LastWin.moveonly, (int)LastWin.maximize, (int)LastWin.snap, (int)state.blockaltup,
            (int)state.blockmouseup, (int)state.ignorekey);
    fclose(f);
}
...
#    pragma comment(linker, "/EXPORT:" __FUNCTION__ "=" __FUNCDNAME__)
    ...
    if (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) {
        if (!state.alt && !state.action && (!conf.KeyCombo || (state.alt1 && state.alt1 != vkey)) && IsHotkey(vkey)
...
// Stupid software that like to steal Alt.
        ...
        } else if (conf.UniKeyHoldMenu && (fhwnd = GetForegroundWindow()) && !IsFullScreenBL(fhwnd)
                   && !blacklisted(fhwnd, &BlkLst.Processes) && !blacklisted(fhwnd, &BlkLst.Windows)) {
            // Key lists used below...
            static const UCHAR ctrlaltwinkeys[]  = {VK_CONTROL, VK_MENU, VK_LWIN, VK_RWIN, 0};
            static const UCHAR menupopdownkeys[] = {VK_BACK,  VK_TAB,   VK_APPS, VK_DELETE, VK_SPACE, VK_LEFT,
                                                    VK_RIGHT, VK_PRIOR, VK_NEXT, VK_END,    VK_HOME,  0};
...
static int ScrollPointedWindow(POINT pt, int delta, WPARAM wParam)
{
    ...
    for (i = 0; i < ARR_SZ(toOr); i++) {  // Should we use GetKeyState?
        if (GetAsyncKeyState(toOr[i]) & 0x8000) {
            wp |= (1 << i);
        }
    }
...
static int IsToolWindow(HWND hwnd) { return GetWindowLongPtr(hwnd, GWL_EXSTYLE) & WS_EX_TOOLWINDOW; }
/////////////////////////////////////////////////////////////////////////////
...
BOOL CALLBACK EnumAltTabWindows(HWND window, LPARAM lParam)
{
    ...
    if (IsAltTabAble(window) && (!IsIconic(window) || (lParam && !IsToolWindow(window)))
        && state.origin.monitor
...
BOOL CALLBACK EnumTopMostWindows(HWND window, LPARAM lParam)
{
    ...
    // Only store window if it's visible, not minimized
    // to taskbar and on the same monitor as the cursor
    if (IsAltTabAble(window) && GetWindowLongPtr(window, GWL_EXSTYLE) & WS_EX_TOPMOST) {
        hwnds[numhwnds++] = window;
    }
    return TRUE;
}
...
BOOL CALLBACK EnumStackedWindowsProc(HWND hwnd, LPARAM lasermode)
{
    ...
    // Only store window if it's visible, not minimized to taskbar
    RECT wnd, refwnd;
    if (IsAltTabAble(hwnd) && !IsIconic(hwnd) && GetWindowRectL(state.hwnd, &refwnd) && GetWindowRectL(hwnd, &wnd)
...
static short ScaleDeltaAndAccum(short delta, short tar_delta)
{
    ...
    // Only accumulate
    if (step == 0) {  // step is too small, we need to accumulate delta
...
static const GUID my_IID_IAudioEndpointVolume = {
...
static HRESULT(WINAPI *myCoCreateInstance)(REFCLSID rclsid, LPUNKNOWN pUnkOuter, DWORD dwClsContext, REFIID riid,
                                           LPVOID *ppv);
...
static BOOL LoadOleDll()
{
    ...
    myCoCreateInstance =
        (HRESULT(WINAPI *)(REFCLSID, LPUNKNOWN, DWORD, REFIID, LPVOID *))GetProcAddress(h, "CoCreateInstance");
...
static HRESULT GetCurrentVolumeMute(UINT *curentVol, UINT *maxVol, BOOL *muted)
{
    ...
    if (osver >= 6 && LoadOLEDLLOnce()) {
        ...
        hr = myCoCreateInstance(&my_CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL, &my_IID_IMMDeviceEnumerator,
                                (void **)&pDevEnum);
        if (hr != S_OK) {
            goto fail;
        }
...
#endif  // NO_OLEAPI
    while (num--) { Send_KEY(delta > 0 ? VK_VOLUME_UP : VK_VOLUME_DOWN); }
}
...
static void SetBottomMost(HWND hwnd)
{
    ...
    if (!blacklisted(hwnd, &BlkLst.Bottommost)) {
        ...
        while ((tmph = GetNextWindow(tmph, GW_HWNDNEXT)) && i++ < 1024) {
            if (IsVisible(tmph) && !IsIconic(tmph) && blacklisted(tmph, &BlkLst.Bottommost)) {
    ...
    if (delta > 0) {
        ...
        if (shift) {
            ...
            if (hwnd == GetAncestor(GetForegroundWindow(), GA_ROOT)) {
                ...
                if (!tmp) {
                    tmp = GetWindow(hwnd, GW_HWNDPREV);
                }
...
// Works oly for Desktop monitors.
    ...
    if (dll) {
        ...
        myGetPhysMonitorsFromHM =
            (BOOL(WINAPI *)(HMONITOR, DWORD, LPPHYSICAL_MONITOR))GetProcAddress(dll, "GetPhysicalMonitorsFromHMONITOR");
        myGetMonitorBrightness =
            (BOOL(WINAPI *)(HANDLE, LPDWORD, LPDWORD, LPDWORD))GetProcAddress(dll, "GetMonitorBrightness");
        mySetMonitorBrightness = (BOOL(WINAPI *)(HANDLE, DWORD))GetProcAddress(dll, "SetMonitorBrightness");
        myDestroyPhysicalMonitors =
            (BOOL(WINAPI *)(DWORD, LPPHYSICAL_MONITOR))GetProcAddress(dll, "DestroyPhysicalMonitors");
        myGetNumberOfPhysmons =
            (BOOL(WINAPI *)(HMONITOR, LPDWORD))GetProcAddress(dll, "GetNumberOfPhysicalMonitorsFromHMONITOR");
        myGetMonitorCapabilities =
            (BOOL(WINAPI *)(HANDLE, LPDWORD, LPDWORD))GetProcAddress(dll, "GetMonitorCapabilities");

        if (!(myGetPhysMonitorsFromHM && myGetMonitorBrightness && mySetMonitorBrightness && myDestroyPhysicalMonitors
...
static HCURSOR CursorToDraw()
{
    ...
    if (state.action == AC_MOVE) {
        ...
        if (!cursor) {
            cursor = LoadCursor(NULL, IDC_ARROW);  // Fallback;
        }
        return cursor;
    }
...
static void RollWindow(HWND hwnd, int delta)
{
    ...
    if (IsZoomed(hwnd)) {
        ...
        if (delta <= 0 && ismxrolled) {
            ...
            SetWindowPos(hwnd, NULL, 0, 0, rc.right - rc.left, GetSystemMetrics(SM_CYMIN),
...
static int ActionZoom(HWND hwnd, short delta, short center)
{
    ...
    UCHAR CT  = !center ^ !state.ctrl || (state.resize.x == RZ_XCENTER && state.resize.y == RZ_YCENTER);

    if (!conf.AutoSnap
        || (CT
            && (state.resize.x == RZ_XCENTER
                || state.resize.y
                       == RZ_YCENTER))) {  // Try to conserve a bit better the aspect ratio when in center mode.
    ...
    ResizeSnap(&x, &y, &width, &height, min(max(left, right) - 1, conf.SnapThreshold)  // initial x threshold
               ,
    ...
    if (state.resize.x == RZ_LEFT) {
        x = x + width - CLAMPW(width);
    }
    if (state.resize.y == RZ_TOP) {
        y = y + height - CLAMPH(height);
    }
    // Avoid runaway effect when zooming in/out too much.
    if (state.resize.x == RZ_XCENTER && !ISCLAMPEDW(width)) {
        x = orc.left;
    }
    if (state.resize.y == RZ_YCENTER && !ISCLAMPEDH(height)) {
        y = orc.top;
    }
...
static void NextBorders(RECT *pos, const RECT *cur, const RECT *def)
{
    ...
    for (i = 0; i < numsnwnds; i++) {
        ...
        POINT          tpt;
        tpt.x = (rc->left + rc->right) / 2;
        tpt.y = (rc->top + rc->bottom) / 2;
        if (!PtInRect(def, tpt)) {
            continue;
        }

        if (flg & (SNZONE | SNLEFT) && rc->right < cur->left) {
            pos->left = max(pos->left, rc->right);
        }
        if (flg & (SNZONE | SNTOP) && rc->bottom < cur->top) {
            pos->top = max(pos->top, rc->bottom);
        }
        if (flg & (SNZONE | SNRIGHT) && rc->left > cur->right) {
            pos->right = min(pos->right, rc->left);
        }
        if (flg & (SNZONE | SNBOTTOM) && rc->top > cur->bottom) {
            pos->bottom = min(pos->bottom, rc->top);
        }
    }
}
...
static void CALLBACK HandleWinEvent(HWINEVENTHOOK hook, DWORD event, HWND hwnd, LONG idObject, LONG idChild,
                                    DWORD dwEventThread, DWORD dwmsEventTime)
{
    HWND pinhwnd = (HWND)hook;  // 1st param...
    if (hwnd && (event == EVENT_OBJECT_DESTROY || event == EVENT_OBJECT_LOCATIONCHANGE) && idChild == INDEXID_CONTAINER
        ...
        case WM_TIMER:
            {
                ...
                // or no longer exists
                LONG_PTR xstyle;
                if (!IsWindow(ow) || !IsWindowVisible(ow)
                ...
                if (OldOwStyle == style) {
                    ...
                    SetWindowPos(hwnd, NULL, rc.right - LOWORDPTR(RTOffset), rc.top + HIWORDPTR(RTOffset), 0, 0,
                ...
                SetWindowPos(hwnd, NULL, rc.right - rightoffset, rc.top + topoffset, PinW, PinH,
...
// TODO: Event Hook
        ...
        // Remove topmost flag if the pin gets destroyed.
                HWND ow;
                if ((ow = GetWindow(hwnd, GW_OWNER)) && IsWindow(ow)
...
static HWND CreatePinWindow(const HWND owner)
{
    ...
    HWND ret = CreateWindowEx(WS_EX_TOOLWINDOW | WS_EX_TOPMOST, TEXT(APP_NAMEA) TEXT("-Pin"), NULL,
                              WS_POPUP | WS_BORDER /* Start invisible */
                              ,
                              0, 0, 0, 0, owner, NULL, hinstDLL, NULL);
...
{
    RECT rc, bd, mon;
    if (!IsResizable(hwnd) || !GetWindowRect(hwnd, &rc)) {
        return;
    }
    OffsetRectMDI(&rc);
    ...
    if (horizontal) {
        ...
        MoveWindowAsync(hwnd, mon.left - bd.left, rc.top, mon.right - mon.left + bd.left + bd.right,
                        rc.bottom - rc.top);
    } else {  // vertical
        SetRestoreFlag(hwnd, SNAPPED | SNMAXH);
        MoveWindowAsync(hwnd, rc.left, mon.top - bd.top, rc.right - rc.left, mon.bottom - mon.top + bd.top + bd.bottom);
    }
}
...
BOOL CALLBACK MinimizeWindowProc(HWND hwnd, LPARAM lParam)
{
    ...
    if (hwnd != p->clickedhwnd
        // && IsAltTabAble(hwnd) ????
        && IsVisible(hwnd) && !IsIconic(hwnd) && (WS_MINIMIZEBOX & GetWindowLongPtr(hwnd, GWL_STYLE))) {
...
static void MinimizeAllOtherWindows(HWND hwnd, int CurrentMonOnly)
{
    ...
    if (restore == hwnd) {
        ...
        for (i = 0; i < numminhwnds; i++) {
            ...
            if (IsWindow(hrest) && IsIconic(hrest)
...
static void TrackMenuOfWindows(WNDENUMPROC EnumProc, LPARAM flags)
{
    ...
    if (!g_mchwnd) {
        return;  // Failed to create g_mchwnd...
    }
...
// Fill up MSAA structure for screen readers.
    ...
    i = (unsigned)TrackPopupMenu(menu, TPM_RETURNCMD /*|TPM_NONOTIFY*/ | GetSystemMetrics(SM_MENUDROPALIGNMENT), pt.x,
                                 pt.y, 0, g_mchwnd, NULL);
...
static void ActionMenu(HWND hwnd)
{
    ...
    PostMessage(g_mainhwnd, WM_SCLICK, (WPARAM)g_mchwnd,
...
static BOOL CALLBACK FindTiledWindowEnumProc(HWND hwnd, LPARAM lp)
{
    ...
    if (tw->diagonal) {
        ...
        switch (tw->direction) {
            case 0:           // LEFT
                if (dx < 0 && ady <= adx && (adx < tw->distance.x || (adx == tw->distance.x && ady < tw->distance.y))) {
                    break;    // Window is closer...
                }
                return TRUE;  // skip
            case 1:           // UP
                if (dy < 0 && adx <= ady && (ady < tw->distance.y || (ady == tw->distance.y && adx < tw->distance.x))) {
                    break;
                }
                return TRUE;
            case 2:  // RIGHT
...
static void SClickActions(HWND hwnd, enum action action)
{
    ...
    switch (action) {
        ...
        case AC_MAXIMIZE:
            ...
            break;
        case AC_CENTER:
            CenterWindow(hwnd, !state.shift /*state.shift? 0: CW_RESTORE*/);
            break;
        case AC_ALWAYSONTOP:
            TogglesAlwaysOnTop(hwnd);
            break;
        case AC_CLOSE:
            ...
            break;
        case AC_LOWER:
            ActionLower(hwnd, 0, state.shift, state.ctrl);
            break;
        case AC_FOCUS:
            ActionLower(hwnd, +120, state.shift, 1);
            break;
        case AC_BORDERLESS:
        ...
        case AC_ROLL:
            ...
            break;
        case AC_MAXHV:
            MaximizeHV(hwnd, state.shift);
            break;
        case AC_MINALL:
            MinimizeAllOtherWindows(hwnd, state.shift);
            break;
        case AC_MUTE:
            Send_KEY(VK_VOLUME_MUTE);
            break;
        case AC_SIDESNAP:
            SnapToCorner(hwnd, AUTORESIZE, !!state.shift);
            break;
        case AC_EXTENDSNAP:
            SnapToCorner(hwnd, AUTORESIZE, !state.shift);
            break;
        case AC_EXTENDTNEDGE:
            SnapToCorner(hwnd, AUTORESIZE, state.shift ? SNTO_MOVETO | SNTO_NEXTBD : SNTO_EXTEND | SNTO_NEXTBD);
            break;
        case AC_MOVETNEDGE:
            SnapToCorner(hwnd, AUTORESIZE, state.shift ? SNTO_EXTEND | SNTO_NEXTBD : SNTO_MOVETO | SNTO_NEXTBD);
            break;
        case AC_MENU:
            ActionMenu(hwnd);
            break;
        case AC_NSTACKED:
            ActionAltTab(state.prevpt, +120, state.shift, EnumStackedWindowsProc);
            break;
        case AC_NSTACKED2:
            ActionAltTab(state.prevpt, +120, !state.shift, EnumStackedWindowsProc);
            break;
        ...
        case AC_PSTACKED2:
            ...
            break;
        case AC_STACKLIST:
            ActionStackList(state.shift ? TRK_LASERMODE : 0);
            break;
        case AC_STACKLIST2:
            ActionStackList(state.shift ? 0 : TRK_LASERMODE);
            break;
        case AC_ALTTABLIST:
            PostMessage(g_hkhwnd, WM_STACKLIST, TRK_MOVETOMONITOR | TRK_LASERMODE,
                        state.shift ? (LPARAM)EnumAllAltTabWindows : (LPARAM)EnumAltTabWindows);
            break;
        case AC_ALTTABFULLLIST:
            PostMessage(g_hkhwnd, WM_STACKLIST, TRK_MOVETOMONITOR | TRK_LASERMODE,
                        state.shift ? (LPARAM)EnumAltTabWindows : (LPARAM)EnumAllAltTabWindows);
            break;
        case AC_MLZONE:
            MoveWindowToTouchingZone(hwnd, 0, 0);
            break;  // mLeft
        case AC_MTZONE:
            MoveWindowToTouchingZone(hwnd, 1, 0);
            break;  // mTop
        case AC_MRZONE:
            MoveWindowToTouchingZone(hwnd, 2, 0);
            break;  // mRight
        case AC_MBZONE:
            MoveWindowToTouchingZone(hwnd, 3, 0);
            break;  // mBottom
        case AC_XLZONE:
            MoveWindowToTouchingZone(hwnd, 0, 1);
            break;  // xLeft
        case AC_XTZONE:
            MoveWindowToTouchingZone(hwnd, 1, 1);
            break;  // xTop
        case AC_XRZONE:
            MoveWindowToTouchingZone(hwnd, 2, 1);
            break;  // xRight
        case AC_XBZONE:
            MoveWindowToTouchingZone(hwnd, 3, 1);
            break;  // xBottom
        case AC_XTNLEDGE:
            SnapToCorner(hwnd, RXY_LEFT_CENTER, SNTO_EXTEND | SNTO_NEXTBD);
            break;
        case AC_XTNTEDGE:
            SnapToCorner(hwnd, RXY_CENTER_TOP, SNTO_EXTEND | SNTO_NEXTBD);
            break;
        case AC_XTNREDGE:
            SnapToCorner(hwnd, RXY_RIGHT_CENTER, SNTO_EXTEND | SNTO_NEXTBD);
            break;
        case AC_XTNBEDGE:
            SnapToCorner(hwnd, RXY_CENTER_BOTTOM, SNTO_EXTEND | SNTO_NEXTBD);
            break;
        case AC_MTNLEDGE:
            SnapToCorner(hwnd, RXY_LEFT_CENTER, SNTO_MOVETO | SNTO_NEXTBD);
            break;
        case AC_MTNTEDGE:
            SnapToCorner(hwnd, RXY_CENTER_TOP, SNTO_MOVETO | SNTO_NEXTBD);
            break;
        case AC_MTNREDGE:
            SnapToCorner(hwnd, RXY_RIGHT_CENTER, SNTO_MOVETO | SNTO_NEXTBD);
            break;
        case AC_MTNBEDGE:
            SnapToCorner(hwnd, RXY_CENTER_BOTTOM, SNTO_MOVETO | SNTO_NEXTBD);
            break;
        case AC_STEPL:
            StepWindow(hwnd, -conf.KBMoveStep, 0);
            break;
        case AC_STEPT:
            StepWindow(hwnd, -conf.KBMoveStep, 1);
            break;
        case AC_STEPR:
            StepWindow(hwnd, +conf.KBMoveStep, 0);
            break;
        case AC_STEPB:
            ...
            break;
        case AC_SSTEPL:
            StepWindow(hwnd, -conf.KBMoveSStep, 0);
            break;
        case AC_SSTEPT:
            StepWindow(hwnd, -conf.KBMoveSStep, 1);
            break;
        case AC_SSTEPR:
            StepWindow(hwnd, +conf.KBMoveSStep, 0);
            break;
        case AC_SSTEPB:
            ...
            break;
        case AC_FOCUSL:
            ReallySetForegroundWindow(FindTiledWindow(hwnd, 0));
            break;
        case AC_FOCUST:
            ReallySetForegroundWindow(FindTiledWindow(hwnd, 1));
            break;
        case AC_FOCUSR:
            ReallySetForegroundWindow(FindTiledWindow(hwnd, 2));
            break;
        case AC_FOCUSB:
            ReallySetForegroundWindow(FindTiledWindow(hwnd, 3));
            break;
...
static int DoWheelActions(HWND hwnd, enum action action)
{
    ...
    switch (action) {
        case AC_ALTTAB:
            ActionAltTab(state.prevpt, state.delta, /*laser=0*/ 0,
                         state.shift ? EnumStackedWindowsProc : EnumAltTabWindows);
            break;
        case AC_VOLUME:
            ActionVolume(state.delta);
            break;
        case AC_TRANSPARENCY:
            ret = ActionTransparency(hwnd, state.delta);
            break;
        case AC_LOWER:
            ActionLower(hwnd, state.delta, state.shift, state.ctrl);
            break;
        case AC_MAXIMIZE:
            ActionMaxRestMin(hwnd, state.delta);
            break;
        case AC_ROLL:
            RollWindow(hwnd, state.delta);
            break;
        case AC_HSCROLL:
            ret = ScrollPointedWindow(state.prevpt, -state.delta, WM_MOUSEHWHEEL);
            break;
        case AC_ZOOM:
            ret = ActionZoom(hwnd, state.delta, 0);
            break;
        case AC_ZOOM2:
            ret = ActionZoom(hwnd, state.delta, 1);
            break;
        case AC_NPSTACKED:
            ActionAltTab(state.prevpt, state.delta, state.shift, EnumStackedWindowsProc);
            break;
        case AC_NPSTACKED2:
            ActionAltTab(state.prevpt, state.delta, !state.shift, EnumStackedWindowsProc);
            break;
        ...
        default:
            {
                ...
                if (AC_SHRT0 <= rac && rac < AC_SHRT0 + ARR_SZ(conf.inputSequences)
                    ...
                    ret = 1;
                    SendInputSequence(conf.inputSequences[rac - AC_SHRT0]);
                    break;
                }
            }
            break;
...
static int init_movement_and_actions(POINT pt, HWND hwnd, enum action action, int buttonX)
{
    ...
    if (blacklisted(state.hwnd, &BlkLst.Processes) || isClassName(state.hwnd, TEXT(APP_NAMEA) TEXT("-Pin"))
        || (blacklisted(state.hwnd, &BlkLst.Windows) && !state.hittest && button != BT_WHEEL
            && button != BT_HWHEEL)  // does not apply in titlebar, nor for the wheel action...
        || GetWindowPlacement(state.hwnd, &wndpl) == 0 || GetWindowRect(state.hwnd, &wnd) == 0
    ...
    if (MOUVEMENT(action)) {
        ...
        if (conf.DragSendsAltCtrl && !(GetKeyState(VK_MENU) & 0x8000) && !(GetKeyState(VK_SHIFT) & 0x8000)) {
        ...
        if (state.hittest == 2 && button == BT_LMB) {
            return 0;
        }
...
static int xpure IsAeraCapbutton(int area)
{
    return area == HTMINBUTTON || area == HTMAXBUTTON || area == HTCLOSE || area == HTHELP;
}
...
{
    int willtest = ((conf.TTBActions & 1) && !state.alt) || ((conf.TTBActions & 2) && state.alt);
    if (willtest && action) {
...
static void FinishMovement()
{
    ...
    StopSpeedMes();
    //    Sleep(5000);
    if (LastWin.hwnd && (state.moving == NOT_MOVED || (!conf.FullWin && state.moving == 1))) {
    ...
    if (conf.AutoRemaximize && state.moving && (state.origin.maximized || state.origin.fullscreen) && !state.shift
...
static xpure int GetButton(WPARAM wp, LPARAM lp)
{
    ...
    return (wp == WM_LBUTTONDOWN || wp == WM_LBUTTONUP)   ? BT_LMB
...
{
    return (wp == WM_LBUTTONDOWN || wp == WM_MBUTTONDOWN || wp == WM_RBUTTONDOWN || wp == WM_XBUTTONDOWN) ? STATE_DOWN
           : (wp == WM_LBUTTONUP || wp == WM_MBUTTONUP || wp == WM_RBUTTONUP || wp == WM_XBUTTONUP)       ? STATE_UP
...
LRESULT CALLBACK LowLevelMouseProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    if (buttonstate == STATE_DOWN && state.alt) {
        ...
        if (!state.action && !IsHotclick(state.alt) && !IsHotkeyDown()) {
            UnhookMouse();
            return CALLNEXTHOOK;  // CallNextHookEx(NULL, nCode, wParam, lParam);
        }
        ...
        if (action && (state.action == action || (state.action == AC_MOVE && action == AC_RESIZE))
            ...
            if (action == AC_MOVE) {
                action = conf.MoveUp[ModKey() + inTTB];
            }
            if (action == AC_RESIZE) {
                action = conf.ResizeUp[ModKey() + inTTB];
            }
...
{
    return IsAeraCapbutton(area) || area == HTHSCROLL || area == HTVSCROLL || area == HTSYSMENU;
}
/////////////////////////////////////////////////////////////////////////////
            ...
            case GRAB_TIMER:
                {
                    ...
                    if (IsSamePTT(&pt, &state.clickpt)
                        ...
                        && (ptwnd = WindowFromPoint(pt)) && !IsAreaLongClikcable(HitTestTimeoutbl(ptwnd, pt))) {
                        ...
                        if (ret) {          // Release mouse click if we have to move.
                            InterlockedIncrement(&state.ignoreclick);
                            mouse_event(buttonswaped ? MOUSEEVENTF_RIGHTUP : MOUSEEVENTF_LEFTUP, 0, 0, 0,
                                        GetMessageExtraInfo());
                            InterlockedDecrement(&state.ignoreclick);
...
static LRESULT DrawMenuItem(HWND hwnd, WPARAM wParam, LPARAM lParam, UINT dpi, HFONT mfont)
{
    ...
    DrawIconEx(di->hDC, di->rcItem.left + xmargin, di->rcItem.top + yicooffset,
               (di->itemState & ODS_GRAYED) ? LoadIcon(NULL, IDI_HAND) : data->icon, xicosz, yicosz, 0, NULL,
               DI_NORMAL);
...
{
    if (conf.RCCloseMItem && 0 <= id && (UINT)id < numhwnds && GetWindowLongPtr(hwnd, GWLP_USERDATA) == 3
        && IsWindow(hwnds[id])) {
        ...
        case WM_COMMAND:
            {
                if (LOWORD(wParam)) {
                    ...
                    if (HIWORD(wParam)) {
                        SendUnicodeKey(HIWORD(wParam));  // Upper surrogate
                    }
                    ...
                    if (action) {
                        ...
                        if (action == AC_LOWER || action == AC_MINIMIZE || action == AC_KILL || action == AC_CLOSE) {
                            fhwndori = NULL;
                        }
                    }
                    // Menu closes now.
        ...
        case WM_MENUCHAR:
            {
                ...
                if (GetWindowLongPtr(hwnd, GWLP_USERDATA) == 3) {
                    ...
                    if (conf.NumberMenuItems) {
                        ...
                        // O-9 then A-Z
                        item = ('0' <= c && c <= '9') ? c - '0' : ('a' <= c && c <= 'z') ? c - 'a' + 10 : 0xFFFF;
                    } else {
                        // A-Z then 0-9
                        // If UPPERCASE
                        closewindow = conf.RCCloseMItem && 'A' <= cc && cc <= 'Z' && GetKeyState(VK_SHIFT) & 0x8000;
                        item        = ('a' <= c && c <= 'z') ? c - 'a' : ('0' <= c && c <= '9') ? c - '0' + 26 : 0xFFFF;
                    }
        ...
        case WM_DESTROY:
            {
                ...
                if (fhwndori && state.sclickhwnd == fhwndori && GetWindowLongPtr(hwnd, GWLP_USERDATA) == 1
                    && IsWindow(fhwndori)) {
...
#    pragma comment(linker, "/EXPORT:" __FUNCTION__ "=" __FUNCDNAME__)
    ...
    unsigned ac;
    for (ac = AC_MENU; ac < AC_MAXVALUE; ac++) { UnregisterHotKey(g_hkhwnd, 0xC000 + ac); }
...
static void readblacklist(const TCHAR *section, struct blacklist *blacklist, const char *bl_str)
{
    ...
    while (pos) {
        ...
        blacklist->items =
            (struct blacklistitem *)realloc(blacklist->items, (blacklist->length + 1) * sizeof(struct blacklistitem));
        if (!blacklist->items) {
...
static unsigned readhotkeys(const TCHAR *inisection, const char *name, const TCHAR *def, UCHAR *keys, unsigned MaxKeys)
{
    ...
    while (*pos) {
        ...
        while (*pos && *pos >= '0') {
            pos++;  // go to the end of the word
        }
        while (*pos && *pos < '0') {
            pos++;  // go to next char after spaces.
        }
    }
    keys[i] = 0;
    return i;
...
void readbuttonactions(const TCHAR *inputsection)
{
    static const char *buttons[] = {
        "LMB",         "RMB",    "MMB",      "MB4",  "MB5",  "MB6",  "MB7",  "MB8",  "MB9",  "MB10",   "MB11",
        "MB12",        "MB13",   "MB14",     "MB15", "MB16", "MB17", "MB18", "MB19", "MB20", "Scroll", "HScroll",

        "GrabWithAlt", "MoveUp", "ResizeUp",
    };
    ...
    for (i = 0; i < ARR_SZ(buttons); i++) {
        ...
        key[len]                 = 'B';
        key[len + 1]             = '\0';  // Secondary B sufixe
        actionptr[NACPB * i + 1] = readaction(inputsection, key);

        // Titlbar actions
        key[len]                 = 'T';
        key[len + 1]             = '\0';  // Titlebar T sufixes
        ...
        // Action while moving
        key[len]                 = 'M';
        key[len + 1]             = '\0';  // Action while Moving M sufixe
...
static HWND KreateMsgWin(WNDPROC proc, const TCHAR *name, LONG_PTR userdata)
{
    ...
    HWND hwnd   = CreateWindowEx(0, wnd.lpszClassName, NULL, 0, 0, 0, 0, 0, parent, NULL, hinstDLL, NULL);
    if (hwnd && userdata) {
...
static void CreateTransWin(const TCHAR *inisection)
{
    ...
    g_transhwnd[0] = NULL;
    if (conf.TransWinOpacity) {
        int xflags = conf.TransWinOpacity == 255 ? WS_EX_TOPMOST | WS_EX_TOOLWINDOW
                                                 : WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_LAYERED;
        g_transhwnd[0] =
            CreateWindowEx(xflags, wnd.lpszClassName, NULL, WS_POPUP, 0, 0, 0, 0, NULL, NULL, hinstDLL, NULL);
    ...
    if (!g_transhwnd[0]) {
        ...
        for (i = 0; i < 4; i++) {  // the transparent window is made with 4 thin windows
            g_transhwnd[i] = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TOOLWINDOW, wnd.lpszClassName, NULL, WS_POPUP, 0, 0,
                                            0, 0, g_mainhwnd, NULL, hinstDLL, NULL);
...
static void readalluchars(UCHAR *dest, const TCHAR *const inisection, const struct OptionListItem *optlist,
                          size_t listlen)
{
    // Read all char options
    unsigned i;
    for (i = 0; i < listlen; i++) { *dest++ = GetSectionOptionInt(inisection, optlist[i].name, optlist[i].def); }
}
...
{
    size_t i;
    for (i = 0; i < ARR_SZ(conf.inputSequences); i++) { free(conf.inputSequences[i]); }
}
...
#    pragma comment(linker, "/EXPORT:" __FUNCTION__ "=" __FUNCDNAME__)
    ...
    if (conf.SidesFraction == 255) {
        conf.SidesFraction = conf.CenterFraction;
    }
    ...
    if (conf.RezTimer) {
        conf.RefreshRate = 0;    // Ignore the refresh rate in RezTimer mode.
    }
    ...
    ResetDB();  // Zero database of restore info (snap.inl)
